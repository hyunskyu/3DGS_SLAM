...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, mask=None):
    ...
    if mask is not None:
        rendered_color, radii, rendered_depth, opacity = rasterizer(
        ...
        rendered_color, radii, rendered_depth, opacity, n_touched = rasterizer(
            means3D=means3D,
            means2D=means2D,
            shs=shs,
    ...
    #return rendered_image,screenspace_points, radii > 0,radii,depth,opacity,n_touched
